...
if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    echo '{"result": "success", "systemMessage": "Not a git repository."}'
    exit 0
fi
...
STAGED=$(echo "$STATUS" | grep -c '^[MADR]' 2>/dev/null || echo 0)
...
jq -n --arg msg "$SUMMARY" '{"result":"success","systemMessage":$msg}'
